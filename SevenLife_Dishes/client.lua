...
local pedloaded = 0
local pedarea = false
local ped = GetHashKey("a_m_m_socenlat_01")
Citizen.CreateThread(
    function()
        while true do
            ...
            for k, v in pairs(Config.Dishes) do
                    ...
                    if distance < 2 then
                        ...
                        if notifys then
                            TriggerEvent(
                                ...
                                "Drücke <span1 color = white>E</span1> um Dishes zu spielen",
                                "System - Nachricht",
                                true
                            )
                        end
...
RegisterNUICallback(
    "ZuwenigGeld",
    function()
        TriggerEvent("SevenLife:TimetCustom:Notify", "Casino", "Du besitzt zu wenig Chips", 1500)
    end
)
RegisterNUICallback(
    "Verloren",
    function(data)
        TriggerServerEvent("SevenLife:Dishes:Verloren", data.einsatz, data.multi)
    end
)
RegisterNUICallback(
    "Gewonnen",
    function(data)
        TriggerServerEvent("SevenLife:Dishes:Gewonnen", data.einsatz, data.multi)
    end
)
RegisterNetEvent("SevenLife:Dishes:UpdateAll")
AddEventHandler(
    "SevenLife:Dishes:UpdateAll",
    function(coins, list)
        SendNUIMessage(
            {
                type = "UpdateNUI",
