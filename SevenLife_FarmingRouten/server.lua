    ...
    function(source, cb, item)
        ...
        if sand < 50 then
            if item >= 1 then
                ...
                TriggerClientEvent("SevenLife:Sand:SendNotify", _source, "Du brauchst einen Spaten")
            end
        else
            if sand >= 51 then
                TriggerClientEvent("SevenLife:Sand:SendNotify", _source, "Du hast in deinem Rucksack kein Platz")
            end
        end
    end
)
ESX.RegisterServerCallback(
    ...
    function(source, cb, item)
        ...
        if glas < 50 then
            if item >= 3 then
                xPlayer.removeInventoryItem("sand", 3)
                cb(true)
            else
                TriggerClientEvent("SevenLife:Sand:SendNotify", _source, "Du hast zu wenig Sand")
            end
        else
            TriggerClientEvent("SevenLife:Sand:SendNotify", _source, "Du hast in deinem Rucksack kein Platz")
        end
