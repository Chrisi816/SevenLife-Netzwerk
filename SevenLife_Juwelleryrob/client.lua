            ...
            if injuweliermarker then
                if IsControlJustPressed(0, 38) then
                    if not inmenujuwellier then
                        ...
                        clothescoord = GetOffsetFromEntityInWorldCoords(ped, -0.3, 0.4, 0.1)
                        glasses = GetOffsetFromEntityGivenWorldCoords(ped, 0.6, 0.65, 0.6)
                        ear = GetOffsetFromEntityGivenWorldCoords(ped, 0.4, 0.65, 0.6)
                        watch = GetOffsetFromEntityGivenWorldCoords(ped, 0.0, 0.65, 0.6)
...
RegisterNUICallback(
    ...
    function(data)
        ...
        elseif data.chrisi == 3 then
            local decals_1 = GetNumberOfPedDrawableVariations(ped, 10) - 1
            local decals_2 = GetNumberOfPedTextureVariations(ped, 10, "watch_1")
            SendNUIMessage(
                {
...
RegisterNUICallback(
    ...
    function(data)
        ...
        if ChangedValue(maske, data.endvalue1) then
            ...
            local retval1 = GetNumberOfPedPropTextureVariations(player, 1, "ears_1")
            SendNUIMessage(
                {
                    type = "ColorId",
                    colorid = retval1
    ...
    function()
        while true do
            ...
            if isOnCreator == true then
                ...
                elseif plano == "ear" then
                    SetCamCoord(camara, ear)
                elseif plano == "watch" then
                    SetCamCoord(camara, watch)
                end
        ...
        ESX.TriggerServerCallback(
            ...
            function(enoughmoney)
                if enoughmoney then
                    ...
                    TriggerEvent("SevenLife:TimetCustom:Notify", "Juwelier", "Brille Erfolgreich gekauft", 2000)
                else
                    TriggerEvent("SevenLife:TimetCustom:Notify", "Juwelier", "Du besitzt zu wenig Geld", 2000)
                    Resetskin()
                end
            end,
            200
        ...
        ESX.TriggerServerCallback(
            ...
            function(enoughmoney)
                if enoughmoney then
                    ...
                    TriggerEvent("SevenLife:TimetCustom:Notify", "Juwelier", "Ohrring Erfolgreich gekauft", 2000)
                else
                    TriggerEvent("SevenLife:TimetCustom:Notify", "Juwelier", "Du besitzt zu wenig Geld", 2000)
                    Resetskin()
                end
        ...
        ESX.TriggerServerCallback(
            ...
            function(enoughmoney)
                if enoughmoney then
                    ...
                    TriggerEvent("SevenLife:TimetCustom:Notify", "Juwelier", "Uhr Erfolgreich gekauft", 2000)
                else
                    TriggerEvent("SevenLife:TimetCustom:Notify", "Juwelier", "Du besitzt zu wenig Geld", 2000)
                    Resetskin()
                end
        ...
        ESX.TriggerServerCallback(
            ...
            function(enoughmoney)
                if enoughmoney then
                    ...
                    TriggerEvent("SevenLife:TimetCustom:Notify", "Juwelier", "Kette Erfolgreich gekauft", 2000)
                else
                    TriggerEvent("SevenLife:TimetCustom:Notify", "Juwelier", "Du besitzt zu wenig Geld", 2000)
                    Resetskin()
                end
...
RegisterNUICallback(
    ...
    function(data)
        local player = GetPlayerPed(-1)
        if ChangedValue(maske3, data.endvalue1) then
            maske3 = tonumber(data.endvalue1)
            TriggerEvent("SevenLife:Char:SetCam", "kopf")

            SetPedPropIndex(player, 2, maske3, maske4, 2)
            local retval1 = GetNumberOfPedPropTextureVariations(player, 1, "ears_1")
            SendNUIMessage(
                {
                    type = "ColorId1",
...
RegisterNUICallback(
    ...
    function(data)
        local player = GetPlayerPed(-1)
        if ChangedValue(maske4, data.endvalue2) then
            maske4 = tonumber(data.endvalue2)
            TriggerEvent("SevenLife:Char:SetCam", "kopf")
            SetPedPropIndex(player, 2, maske3, maske4, 2)
        end
    end
...
RegisterNUICallback(
    ...
    function(data)
        local player = GetPlayerPed(-1)
        if ChangedValue(maske5, data.endvalue1) then
            maske5 = tonumber(data.endvalue1)
            TriggerEvent("SevenLife:Char:SetCam", "ropa")

            SetPedPropIndex(player, 6, maske5, maske6, true)
            local retval1 = GetNumberOfPedPropTextureVariations(player, 1, "watch_1")
            SendNUIMessage(
                {
                    type = "ColorId2",
                    colorid = retval1
...
RegisterNUICallback(
    ...
    function(data)
        local player = GetPlayerPed(-1)
        if ChangedValue(maske6, data.endvalue2) then
            maske6 = tonumber(data.endvalue2)
            TriggerEvent("SevenLife:Char:SetCam", "ropa")
            SetPedPropIndex(player, 6, maske5, maske6, true)
        end
    end
...
RegisterNUICallback(
    ...
    function(data)
        local player = GetPlayerPed(-1)
        if ChangedValue(mask7, data.endvalue1) then
            mask7 = tonumber(data.endvalue1)
            TriggerEvent("SevenLife:Char:SetCam", "kopf")

            SetPedComponentVariation(player, 7, mask7, maske8, 2)
            local retval1 = GetNumberOfPedPropTextureVariations(player, 1, "watch_1")
            SendNUIMessage(
                {
                    type = "ColorId3",
...
RegisterNUICallback(
    ...
    function(data)
        local player = GetPlayerPed(-1)
        if ChangedValue(maske8, data.endvalue2) then
            maske8 = tonumber(data.endvalue2)
            TriggerEvent("SevenLife:Char:SetCam", "kopf")
            SetPedComponentVariation(player, 7, mask7, maske8, 2)
        end
    end
